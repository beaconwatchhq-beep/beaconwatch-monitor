...
const BUILDING_TYPE_KEYWORDS = [
  ...
  { type: 'industrial', keywords: [
    'manufacturing plant', 'factory', 'industrial facility', 'industrial park',
  ...
  { type: 'office', keywords: [
    'office building', 'office complex', 'professional building', 'business park',
    'high-rise office',
  ] },
...
const COMPILED = BUILDING_TYPE_KEYWORDS.map(({ type, keywords }) => ({
  type,
  res: keywords.map(k => new RegExp(`\\b${escapeRegExp(k)}\\b`, 'i')),
}));
